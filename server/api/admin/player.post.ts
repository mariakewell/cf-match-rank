...
export default defineEventHandler(async (event) => {
  ...
  if (body.id) {
    await db.update(players).set({ name: body.name, groups: body.groups }).where(eq(players.id, Number(body.id)));
  } else {
    await db.insert(players).values({ name: body.name, groups: body.groups, createdAt: new Date() });
  }
